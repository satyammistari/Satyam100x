...
function TypingText({ text, speed = 60, className = "", onDone, erase = false, eraseSpeed = 30 }: TypingTextProps) {
  const [displayed, setDisplayed] = useState(erase ? text : "");
  useEffect(() => {
    let i = erase ? text.length : 0;
    setDisplayed(erase ? text : "");
    if (!erase) {
  ...
  }, [text, speed, erase, eraseSpeed, onDone]);
  return <span className={className}>{displayed}</span>;
}

// Typing loop for skills
const skills = [
  "Frontend Engineer",
...
function SkillTypingLoop() {
  const [index, setIndex] = useState(0);
  const [erase, setErase] = useState(false);

  // Defensive: fallback if skills is empty or index is out of bounds
  const currentSkill = skills && skills.length > 0 && skills[index] ? skills[index] : "Professional Developer";

  return (
    <TypingText
      key={currentSkill + erase}
      text={currentSkill}
      speed={60}
      erase={erase}
      eraseSpeed={30}
      className="text-xl md:text-3xl text-primary font-semibold min-h-[2.5rem] md:min-h-[3rem] mt-2 whitespace-nowrap text-center"
      onDone={() => {
        if (!erase) {
          setTimeout(() => setErase(true), 1800);
        } else {
          setErase(false);
          setIndex((prev) => (prev + 1) % (skills.length || 1));
        }
      }}
    />
  );
    ...
    <section className="relative w-full py-12 md:py-20 px-6 md:px-12 flex flex-col items-center justify-center overflow-hidden bg-background">
      ...
      <div className={`relative z-10 max-w-4xl text-center space-y-6 transition-all duration-700 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="flex flex-col items-center justify-center w-full">
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-bold tracking-tighter text-balance text-foreground flex flex-wrap items-center justify-center gap-2 w-full mb-2">
            Hi, I am <span className="text-primary">Satyam100xDev</span>
          </h1>
          <div className="text-xl md:text-3xl text-primary font-semibold min-h-[2.5rem] md:min-h-[3rem] mt-2 whitespace-nowrap text-center">
            Full-Stack Developer
          </div>
        </div>
        ...
        <div className="pt-6 text-sm text-muted-foreground">
          Open to freelance projects • Remote collaboration
        </div>
      </div>

      {/* Tech Stack Carousel */}
      <div className="w-full max-w-4xl mx-auto mt-10 mb-2 overflow-hidden group">
        <div
          className="flex items-center gap-10 animate-marquee group-hover:[animation-play-state:paused]"
          style={{
