...
export interface Task {
  ...
  liveLink?: string; // Optional live site link
}
...
const TaskCard: React.FC<TaskCardProps> = ({ task, onDragStart, onDragEnd, onStatusChange }) => {
  ...
  return (
    <div
      ...
      className={`task-card p-4 bg-card rounded-xl border border-border shadow-lg hover:shadow-xl transition-all duration-200 h-56 flex flex-col ${isDragging ? 'dragging' : ''}`}
    >
      ...
      {task.image && (
        <div className="mb-2 flex justify-center">
          <img
            src={task.image}
            alt={task.title + ' project screenshot'}
            className="h-24 w-full object-cover rounded-md border border-muted shadow-sm grayscale hover:grayscale-0 transition duration-300"
        ...
        <div className="flex-1 mb-3">
          ...
          {task.liveLink && (
            <a
              ...
              className="inline-block mt-2 px-3 py-1 text-xs font-medium rounded-full bg-primary text-primary-foreground hover:bg-primary/80 transition-colors shadow"
            >
              Live Site ↗
            </a>
          )}
